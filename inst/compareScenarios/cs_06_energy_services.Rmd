...
### ES Pass LDV by tech - perCap - Bar
```{r ES Pass LDV by tech - perCap - Bar}
items <- c(
...
### ES Freight - perCap - Line
```{r ES Freight - perCap - Line}
items <- c(
...
### ES Freight Road by tech - perCap - Bar
```{r ES Freight Road by tech - perCap - Bar}
items <- c(
  "ES|Transport|Freight|Road|BEV pCap",
...
### LDV Vehicles Stock - perCap - EDGE-T
```{r LDV Vehicles Stock - perCap - EDGE-T}
tot <- "Stock|Transport|Pass|Road|LDV pCap"
items <- c(
  "Stock|Transport|Pass|Road|LDV|BEV pCap",
  "Stock|Transport|Pass|Road|LDV|Hybrid electric pCap",
  "Stock|Transport|Pass|Road|LDV|FCEV pCap",
  "Stock|Transport|Pass|Road|LDV|Gases pCap",
  "Stock|Transport|Pass|Road|LDV|Liquids pCap",
  NULL)
showAreaAndBarPlots(data, items, tot, orderVars = "user", scales = "fixed")
```
...
### BOF Steel capacity using process-based steel modeling
```{r Steel BOF cap}
if ("Production|Industry|Steel|BF-BOF" %in% levels(data$variable)) { # if process-based steel is used
  tot <- "Cap|Industry|Steel|BOF"

  #Production|Industry|Steel|+|BF-BOF
  items <- c(
    "Cap|Industry|Steel|BOF")
  # Non-process based scenarios have tot variable, but not items, which leads to errors in display;
  # Therefore, filter data by scenarios that have the 'items' variables.
  dscens <- data %>%
    filter(.data$variable %in% items, .data$scenario != "historical") %>%
...
### BOF Steel capacity additions using process-based steel modeling
```{r Steel BOF Deltacap}
if ("Production|Industry|Steel|BF-BOF" %in% levels(data$variable)) { # if process-based steel is used
  tot <- "New Cap|Industry|Steel|BOF"

  #Production|Industry|Steel|+|BF-BOF
  items <- c(
    "New Cap|Industry|Steel|BOF")
  # Non-process based scenarios have tot variable, but not items, which leads to errors in display;
  # Therefore, filter data by scenarios that have the 'items' variables.
  dscens <- data %>%
    filter(.data$variable %in% items, .data$scenario != "historical") %>%
...
### BOF Steel surviving total capacity after early retirement using process-based steel modeling
```{r Steel BOF Deltacap}
if ("Production|Industry|Steel|BF-BOF" %in% levels(data$variable)) { # if process-based steel is used
  tot <- "Surviving Cap|Industry|Steel|BOF"

  #Production|Industry|Steel|+|BF-BOF
  items <- c(
    "Surviving Cap|Industry|Steel|BOF")
  # Non-process based scenarios have tot variable, but not items, which leads to errors in display;
  # Therefore, filter data by scenarios that have the 'items' variables.
  dscens <- data %>%
    filter(.data$variable %in% items, .data$scenario != "historical") %>%
